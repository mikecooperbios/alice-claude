...
_DEFAULT_TEMPLATES = {
    "teamlead": {
        ...
        "system_prompt": (
            ...
            "4. РЕВЬЮ: проверяешь результаты каждого исполнителя. При низком качестве — "
            "отправляй фидбек HR через send_feedback_to_hr.\n"
    ...
    "hr": {
        ...
        "system_prompt": (
            ...
            "Создаёшь новых агентов через create_agent.\n"
            "При старте сессии — читай pending feedback через get_pending_feedback "
            "и предлагай улучшения на его основе."
        ),
    },
}
...
def create_template(role: str, name: str, system_prompt: str) -> dict:
    with get_conn() as conn:
        conn.execute(
            "INSERT OR REPLACE INTO templates (role, name, system_prompt, updated_at) VALUES (?,?,?,?)",
            (role, name, system_prompt, _now()),
        )
    return get_template(role)
...
def add_feedback(
    ...
    with get_conn() as conn:
        conn.execute(
            "INSERT INTO feedback (role, task_id, project_id, feedback, quality, created_at) "
            "VALUES (?,?,?,?,?,?)",
            (role, task_id, project_id, feedback, quality, _now()),
        )
...
def get_pending_feedback() -> list[dict]:
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM feedback WHERE applied=0 ORDER BY created_at"
        ).fetchall()
    return [dict(r) for r in rows]


def mark_feedback_applied(ids: list[int]) -> None:
    if not ids:
        return
    placeholders = ",".join("?" * len(ids))
    with get_conn() as conn:
        conn.execute(f"UPDATE feedback SET applied=1 WHERE id IN ({placeholders})", ids)
