...
def _is_enabled() -> bool:
    return bool(NOTION_TOKEN and NOTION_PARENT_PAGE_ID and _get_client())
...
TOOL_SCHEMAS = [
    {
        ...
        "input_schema": {
            ...
            "properties": {
                "title": {"type": "string"},
                "idea": {"type": "string", "description": "Идея/описание проекта"},
            },
            "required": ["title", "idea"],
        },
    },
...
def run_tool(name: str, inp: dict) -> str:
    client = _get_client()
    if not client:
        logger.info("Notion disabled — skipping %s", name)
        return f"(Notion отключён — NOTION_TOKEN не задан)"
                ...
                children=[
                    {
                        ...
                        "type": "heading_2",
                        "heading_2": {
                            "rich_text": [{"text": {"content": "Идея проекта"}}]
                        },
                    },
                    {
                        "object": "block",
                        "type": "paragraph",
                        "paragraph": {
                            "rich_text": [{"text": {"content": inp["idea"]}}]
                        },
                    },
                ],
            )
                ...
                children=[
                    {
                        ...
                        "type": "heading_3",
                        "heading_3": {
                            "rich_text": [{"text": {"content": inp["section_title"]}}]
                        },
                    },
                    {
                        "object": "block",
                        "type": "paragraph",
                        "paragraph": {
                            "rich_text": [{"text": {"content": inp["content"][:2000]}}]
                        },
                    },
                ],
            )
