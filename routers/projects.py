...
def approve_project(project_id: str, body: ProjectApprove):
    ...
    # Rejected — re-decompose with feedback
    ...
    message = agents.run_agent(
        ...
        "сохрани новый план через save_plan_summary, опубликуй в Notion если настроен.\n"
...
def notion_check(project_id: str):
    """Read latest Notion comment from the user (polling endpoint)."""
    p = storage.get_project(project_id)
    if not p:
        raise HTTPException(404, "Project not found")
...
def chat(project_id: str, body: AgentChat):
    """Direct chat with the teamlead in context of a project."""
    if not storage.get_project(project_id):
        raise HTTPException(404, "Project not found")
